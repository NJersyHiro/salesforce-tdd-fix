...
description: Use when fixing a Salesforce bug (Flow, Apex, LWC) using TDD — covers requirements gathering, bug reproduction, unit test scenario creation, fix planning with user approval, Notion documentation, implementation, pre-deploy backup, ST execution, screenshot archival, and cleanup, with agent-based review gates between steps.
---
...
**12ステップ構成。ユーザー承認なしに実装を開始してはならない。**
...
| 7 | Notion文書化 | なし |
| 8 | 実装（GREEN）+ デプロイ | GATE-8（code-reviewer） |
| 9 | デプロイ前バックアップ | なし |
| 10 | STシナリオ実行 | GATE-10（test-validator） |
| 11 | スクショ保管 | なし |
| 12 | クリーンアップ | なし |
...
スキルを呼び出したら、**作業開始前に必ず**以下の12タスクをTaskCreateで登録する。

```
TaskCreate × 12（上記ステップ一覧と同じ内容）
```
...
**目的:** 修正内容をNotionに記録する（初心者が読んでも分かる説明を含める）。

```
mcp__claude_ai_Notion__notion-create-pages(
  parent: { page_id: "親ページID" },
  properties: { title: "COMPASS-XXXX: [バグタイトル] 修正記録" },
  children: [...]
)
```

**ページ構成:**
1. **概要（初心者向け）** - 「何が起きていたか」「なぜ問題か」「どう直したか」を3行以内
2. **関連情報** - Backlogチケット URL、対象環境・オブジェクト・フィールド
3. **現状 vs 期待動作** - Step 1の表
4. **テストシナリオ** - Step 3の表
5. **修正内容** - Step 5の方針
6. **テスト結果** - Step 10完了後に追記
7. **注意事項・副作用** - 影響範囲・リスク

> Notionページ作成後、URLをチャットに表示してそのまま次に進む。

---

## Step 8: 実装（GREEN）+ デプロイ

**目的:** Step 5の修正方針に従ってコードを修正し、vpartialにデプロイする。

### デプロイ前の差分確認（必須）

> ⚠️ デプロイ前に必ず対象環境の最新版をリトリーブして差分を確認すること。
...
- [ ] デプロイが成功しているか
- [ ] コンパイルエラー・Flow有効化エラーがないか

---

## Step 9: デプロイ前バックアップ

> ⚠️ Step 8のデプロイを実行する前に必ず実施すること。

```bash
mkdir -p .claude/backup/$(date +%Y-%m-%d)

# ローカルのバックアップ
cp [対象ファイル] .claude/backup/$(date +%Y-%m-%d)/[ファイル名].bak

# vpartial環境の現在の状態もバックアップ
sf project retrieve start --source-dir [対象ファイル] --target-org vpartial
...
Notionのテスト結果セクションにスクショ保管先パスを追記する:
```
mcp__claude_ai_Notion__notion-update-page(page_id: "Step 7で作成したページID", ...)
```
...
## Step 13: コミット・PR作成・マージ

> ⚠️ クリーンアップ（Step 12）完了後に実施すること。
...
| Step 12の前にスクショを削除した | Step 11で `~/screenshots/COMPASS-XXXX-{概要}/` に移動してから削除する |
| sysadminでSTを実施した | Step 10の手順で営業担当ユーザーに切り替える |
